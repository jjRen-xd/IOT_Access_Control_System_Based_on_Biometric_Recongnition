...
#define DEFAULT_DEVICE_RAY  //控制默认输入方式（红外or键盘）
...
bool choice_ray_or_TTP(){//1红外,其他:键盘
  ...
  delay(100);
  char user_choice;
  #ifdef DEFAULT_DEVICE_RAY
    user_choice = get_ray_key();
  #endif
  #ifndef DEFAULT_DEVICE_RAY
    user_choice = get_TTP229_key();
  #endif
  switch (user_choice)
  {
    case '1':
  ...
  if(Serial_terminal.available()>0){//接收终端指令
    switch(data_in = Serial_terminal.read()){
      case '@':
        ...
        #ifdef DEFAULT_DEVICE_RAY //向终端发送选择信息
          get_ray_key();
        #endif
        #ifndef DEFAULT_DEVICE_RAY
          get_TTP229_key();
