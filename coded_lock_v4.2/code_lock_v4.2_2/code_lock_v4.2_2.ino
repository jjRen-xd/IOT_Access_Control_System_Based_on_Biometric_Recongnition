  ...
  while(1){//RESET终止
    ...
    if(Serial.available()>0){//接收人脸识别结果
      ...
      Serial_terminal.print(data = Serial.read());
        ...
        Serial.print(data);
      #endif
      while(Serial.read()>=0){}//清空人脸识别结果的缓存区
    }
  }
}
...
void face_new(){
  ...
  Serial.print('Q');//传输指令
  while(1){
    if(Serial.available()>0){//raspberry有数据就传给arduino3
      data_in = Serial.read();
      Serial_terminal.print(data_in);
      if(data_in == 'W'){
        break;
      }
    }
    if(Serial_terminal.available()>0){//arduino3有数据就传给raspberry,录入成功就结束
      Serial.print((char)Serial_terminal.read());
    }
  }
}

void face_check_number(){
  Serial.print('T');
  if(Serial.available()>0){//raspberry有数据就传给arduino3
