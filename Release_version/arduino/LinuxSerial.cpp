...
{
    char portStr[20];
    sprintf(portStr, "/dev/ttyACM%d", portNo);
...
unsigned char CLinuxSerial::_get_data(){//从arduino2接收数据
    unsigned char data[] = {0xDA};
    CLinuxSerial::ReadData(data,1);
    return data[0];
}
void CLinuxSerial::_send_data(char data){//向arduino2发送数据
    unsigned char send_bytes[] = { 0xFF };
    send_bytes[0] = data;
    CLinuxSerial::WriteData(send_bytes,1);
}
