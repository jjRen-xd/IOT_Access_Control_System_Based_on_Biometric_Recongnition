...
// #define SHOW_IMAGE      //是否显示视频画面
#define SHOW_DEBUG
#define SEND_MESSAGE_TO_ARDUINO //是否与arduino通信
#define SEND_EMAIL          //是否发送邮件，需要sudo运行
#define COLLEDTION_TIMES 15//人脸收集的次数
#define STRANGER_WEIGHT 5
#define NORMAL_WEIGHT 5   //判断权重，值越大越可靠，识别速度越慢
#define RECONGNISE_DISTANCE 0.5  //识别距离
#define NUMBER_OF_SKIP_FRAMES 3   //为保证效率，跳过的帧数
...
int Face::judge_recon_result(std::vector<int> labels,std::vector<int> &names_weight){
    ...
    #ifdef SHOW_DEBUG
        for(size_t i = 0;i<names_weight.size();i++){
            cout<<"names_weight["<<i<<"]:"<<names_weight[i]<<endl;
        }
    #endif
    return -1;
}
...
void Face::record_recon_data(string str,cv::Mat face_record){
    ...
    ALL_RECORD_DATA<<"["<<time_now<<"]-[Name of Unlocker]:"<<str<<"-[Unlocking mode]:人脸解锁"<<endl; 
    NEWEST_DATA<<"["<<time_now<<"]-[Name of Unlocker]:"<<str<<"-[Unlocking mode]:人脸解锁"<<endl; 
    //向arduino发命令开门
    #ifdef SEND_MESSAGE_TO_ARDUINO
        if(str != "stranger"){
    ...
    //发邮件提醒
    #ifdef SEND_EMAIL
        string command = cv::format("echo '有人正在靠近门禁（%s），已经为您抓取人脸，请留意!!!'| mutt -s 门锁检测到人脸 1411102509@qq.com -a %s",str.c_str(),filename.c_str());
...
bool Face::New_face(){
    ...
    //收集人脸
    while(1){
        for(int i = 0;i<NUMBER_OF_SKIP_FRAMES;i++)
            cap>>frame;
        ...
        for(int i = 0;i<facePosition.size();i++){
            ...
            center_x = (2 * facePosition[i].x + facePosition[i].width) / 2;
            center_y = (2 * facePosition[i].y + facePosition[i].height) / 2;
            width = facePosition[i].width;
...
int Face::Recongnise_face(bool times){   //识别人脸，times=1:只成功识别一次（识别多少帧由权重判断）就返回，times=0：无穷
    ...
    while(1){
        ...
        string text = cv::format("FPS:%lf",FPS);
        putText(frame,text,Point(25,25),FONT_HERSHEY_PLAIN,1,Scalar(0,0,255));//显示解算角度
        ...
        if(minDis < RECONGNISE_DISTANCE){   //距离到达一定距离就开始检测记录
            ...
            static std::vector<int> names_weight(Face::size()+1); //将每一帧识别结果格式化存储起来
            ...
            for(int i = 0;i<faceImgs.size();i++){
                ...
                cout<<" angle:("<<faceAngle[i].x<<","<<faceAngle[i].y<<","<<faceAngle[i].z<<")"<<endl;
            }
            for(int i = 0;i<faceLabels.size();i++)
                cout<<"label:"<<faceLabels[i]<<endl<<endl;
        #endif
        #ifdef SHOW_IMAGE
            imshow("face",frame);
...
bool Face::Delete_face(){   //删除人脸，先识别出要删除的对象，然后删除
    #ifdef SEND_MESSAGE_TO_ARDUINO
        arduino._send_data('S');
    #endif
    int delete_Target = Face::Recongnise_face(1);
    cout<<"Prepare to delete face "<<delete_Target<<"!!!"<<endl;
    ...
    //删除存放人脸照片的文件夹
    string command = cv::format("rm -rf ../face_data/s%d",delete_Target);
    std::system(command.c_str());
